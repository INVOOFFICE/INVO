...
/** Résolution de rasterisation (html2canvas) et qualité JPEG — relevées pour un rendu d’impression plus net */
const PDF_HTML2CANVAS_SCALE = 2.75;
...
async function _generateAndDownloadPDF(docObj, tpl, color) {
  ...
  try {
    ...
    if (imgH <= pageH + MIN_SLICE) {
      // Tout tient en une seule page — pas de découpage
...
function buildInvoiceHTML(doc, tpl, bandColor) {
  ...
  function _buildLinesHTML() {
    ...
    <tr style="background:${i % 2 === 1 ? '#f9fafb' : '#fff'}">
      ...
      <td style="padding:8px 10px;text-align:center;font-size:12px;border-bottom:1px solid #f0f0f0">${tvaRate}%</td>
    </tr>`;
      })
      .join('');
    return {
      ...
      theadRow: `
          ...
          <th style="${tableHeaderStyle}padding:9px 10px;text-align:center;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.06em">TVA%</th>`,
    };
  }
  ...
  const { linesHTML, theadRow } = _buildLinesHTML();
...
<div class="page-wrap">
  <div class="page-content">
    ${headerHTML}
    ${clientBlock}
    <table width="100%" cellpadding="0" cellspacing="0" style="border-radius:6px;overflow:hidden;border:1px solid #e5e7eb;margin-bottom:0">
      <thead>
        <tr>
          ${theadRow}
        </tr>
